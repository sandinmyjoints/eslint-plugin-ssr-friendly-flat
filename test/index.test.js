...
ruleTester.run(
  ...
  plugin.rules["no-dom-globals-in-module-scope"],
  {
    valid: [
...
ruleTester.run(
  ...
  plugin.rules["no-dom-globals-in-module-scope"],
  {
    valid: [
      ...
      `interface MyInterface { window: string; }`,
      `interface AnotherInterface { document: boolean; }`,
      `type MyType = { screen: number; }`,
      `type ComplexType = {
      name: string;
      window?: Window;
      config: {
...
ruleTester.run(
  ...
  plugin.rules["no-dom-globals-in-constructor"],
  {
    valid: [
      `class myClass {
      constructor() {}
...
ruleTester.run(
  ...
  plugin.rules["no-dom-globals-in-react-fc"],
  {
    valid: [
      `const Header = () => {
      useEffect(() => {
